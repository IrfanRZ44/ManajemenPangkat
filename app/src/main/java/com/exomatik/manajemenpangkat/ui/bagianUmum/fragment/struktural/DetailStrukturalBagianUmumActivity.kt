package com.exomatik.manajemenpangkat.ui.bagianUmum.fragment.struktural
...
class DetailStrukturalBagianUmumActivity : AppCompatActivity(),
    ...
    private fun uploadFile(data: Uri) {
        ...
        val sRef = mStorageReference?.child("${dataPengajuan?.nip}__${dataPengajuan?.tglPengajuan}/disposisiBagianUmum" + ".pdf")
        sRef?.putFile(data)
    ...
    private fun rejectData(note: String, context: Context){
        ...
        FirebaseDatabase.getInstance().getReference("DataBagianUmum")
    ...
    private fun acceptDisposisi(nip: String, tglPengajuan: String, urlFile: String){
        val tglBagianUmum = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        ...
        FirebaseDatabase.getInstance().getReference("DataBagianUmum")
            .child("UsulanStruktural")
            .child("${dataPengajuan?.nip}__${tglBagianUmum}")
            .setValue(dataPengajuan)
