...
class RiwayatRektorActivity : AppCompatActivity() {
    private lateinit var savedData : DataSave
    private var listProgress = ArrayList<ModelNotifikasiPegawai>()
    private var adapter: AdapterRiwayatRektor? = null
    ...
    private fun myCodeHere(){
        ...
        adapter = AdapterRiwayatRektor(listProgress, this)
            ...
            override fun onDataChange(result: DataSnapshot) {
                ...
                if (result.exists()) {
                    for (snapshot in result.children) {
                        ...
                        if (data != null){
                            if (data.statusPengajuan == "Rektor"){
                                if (data.statusDitolak){
                                    ...
                                    listProgress.add(ModelNotifikasiPegawai(data.tglRektor, "Pengajuan Berkas ${data.nip} (Pelaksana)", data.disposisiRektor, "", 1))
                                    adapter?.notifyDataSetChanged()
                                }
                            }
                        }
                        ...
                        if (data != null){
                            if (data.statusPengajuan == "Rektor"){
                                if (data.statusDitolak){
                                    ...
                                    listProgress.add(ModelNotifikasiPegawai(data.tglRektor, "Pengajuan Berkas ${data.nip} (Struktural)", data.disposisiRektor, "", 1))
                                    adapter?.notifyDataSetChanged()
                                }
                            }
                        }
